        ...
        <div id="tab2" class="tabson">
            ...
            <table class="tablelist">
                ...
                <tr id="id_{{$v->id}}">
                    ...
                    <td>{{$v->id}}</td>
                    <td>{!! str_replace($keyword,"<span style='color:red;display:inline'>$keyword</span>",$v->ad_name) !!}</td>
                    <td>{{$v->role_name}}</td>
                    ...
                    @if($row->id == 1)
                    <td>
                        <a href="{{url('Admin/Manager/managerEdit',$v->id)}}" class="tablelink">编辑</a>
...
<script>
    function delManager(id) {
        ...
        if(bool){
            ...
            $('#'+aid).remove();
            //数据库删除
            $.get("{{url('Admin/Manager/managerDel')}}",{id:id},function ($data) {
                if ($data.status == 1){
                    alert('删除成功!')
                }
