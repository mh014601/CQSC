...
<div class="formbody">
    <div id="usual1" class="usual">
        <div id="tab2" class="tabson">
            <!-- 搜索 -->
            <form action="{{url('Admin/Recycle/goodsRecycle')}}" method="post">
            ...
            <table class="tablelist">
                ...
                @forelse($rows as $v)
                    <tr id="id_{{$v->gid}}">
                        ...
                        <td>
                            <a href="{{url('Admin/Recycle/goodsReduction',[$v->gid])}}" class="tablelink">还原</a>
                            <a href="javascript:void(0)" id="del" onclick="goodsRecycleDel({{$v->gid}})" class="tablelink">删除</a>
                        </td>
                    </tr>
                @empty
                    <tr>
...
<script>
    function goodsRecycleDel(id) {
        var bool = confirm('你确定要删除吗?');
        var _token = '{{csrf_token()}}';
        if (bool){
            $('#id_'+id).remove();
            $.post("{{url('Admin/Recycle/goodsRecycleDel')}}",{id:id,_token:_token},function (data) {
                if (data.status == 1){
                    alert('删除成功!')
                }
            },'json')
